...
1. GitHub fires a webhook when a PR opens or gets a new push.
2. Server verifies the HMAC signature, queues the review, and 200s back fast (GitHub retries anything over ~10s).
3. Reviewer pulls the diff, figures out which Python functions changed, and sends them to Claude with a strict schema.
4. Any finding whose line number falls outside the function's actual range gets dropped on the floor.
5. The rest get posted as inline review comments.
6. Everything lands in Postgres so we can spot when the same bug pattern keeps showing up in the same function.
...
```bash
ANTHROPIC_API_KEY=sk-ant-... PYTHONPATH=. \
...
1. Expose the app publicly. ngrok or a Cloudflare Tunnel is easiest for local testing.
2. Go to the repo's Settings -> Webhooks and add:
   - URL: `https://<your-host>/webhooks/github`
   - Content type: `application/json`
   - Secret: same string as `GITHUB_WEBHOOK_SECRET` in your `.env`
   - Events: Pull requests only
3. `GITHUB_TOKEN` needs the `repo` scope (PAT), or use a GitHub App token with `pull_requests: write` + `contents: read`.
